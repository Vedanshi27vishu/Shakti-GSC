  ...
  try {
    ...
    if (type == 'image') {
      ...
      if (pickedFile != null) {
        ...
        if (!supportedFormats.contains(extension)) {
          _showSnackBar('Unsupported image format. Please select JPG, PNG, GIF, or WebP');
    ...
    var multipartFile = await http.MultipartFile.fromPath(
      ...
      contentType: type == 'image' 
        ? MediaType('image', file.path.split('.').last.toLowerCase() == 'png' ? 'png' : 'jpeg')
        : null,
    );
    ...
    if (responseBody.trim().startsWith('<!DOCTYPE html>') || 
      ...
      if (responseBody.contains('File type not supported')) {
        errorMessage = 'File type not supported by server. Try JPG or PNG format.';
      } else if (responseBody.contains('Only image files are allowed')) {
        errorMessage = 'Only image files are allowed for image upload';
      } else if (responseBody.contains('File too large') || responseBody.contains('413')) {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      if (responseData['success'] == true) {
        ...
        _showSnackBar('${type == 'image' ? 'Image' : 'File'} uploaded successfully');
      } else {
        _showSnackBar('Upload failed: ${responseData['error'] ?? responseData['message'] ?? 'Unknown error'}');
      }
    } else {
      _showSnackBar('Upload failed (${response.statusCode}): ${responseData['error'] ?? responseData['message'] ?? 'Server error'}');
    }
  } catch (e) {
...
Future<void> _uploadFile(File file, String type) async {
  ...
  try {
    String endpoint = type == 'image' ? '/api/upload/image' : '/api/upload/file';
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      if (data['success'] == true) {
        final message = {
          'toUserId': widget.recipientUserId,
          'messageType': type,
          type: data[type], // 'file' or 'image' key
        };
  ...
  try {
    ...
    if (pickedFile != null) {
      ...
      if (!fileName.endsWith('.jpg') && !fileName.endsWith('.jpeg') && 
        ...
        _showSnackBar('Please select a valid image file (jpg, png, gif, webp)');
        setState(() => _isUploading = false);
        return;
      }
                  ...
                  children: List.generate(3, (index) {
                    ...
                    return Container(
                      ...
                      child: Transform.translate(
                        offset: Offset(0, -4 * (animationValue > 0.5 ? 1 - animationValue : animationValue)),
                        child: Container(
                          width: 4,
                          height: 4,
                          decoration: BoxDecoration(
...
Future<void> _openFile(String url) async {
  ...
  Widget _buildEditWidget() {
    if (_editingMessageId == null) return SizedBox.shrink();
