...
void _showEditBusinessDialog() {
  ...
  final businessNameController = TextEditingController(text: ideaDetails['Business_Name'] ?? '');
  final businessSectorController = TextEditingController(text: ideaDetails['Business_Sector'] ?? '');
  final businessCityController = TextEditingController(text: ideaDetails['Business_City'] ?? '');
  final businessLocationController = TextEditingController(text: ideaDetails['Buisness_Location'] ?? '');
  final ideaDescriptionController = TextEditingController(text: ideaDetails['Idea_Description'] ?? '');
  final targetMarketController = TextEditingController(text: ideaDetails['Target_Market'] ?? '');
  final uspController = TextEditingController(text: ideaDetails['Unique_Selling_Proposition'] ?? '');
  
  final startupCostController = TextEditingController(text: financialPlan['Estimated_Startup_Cost']?.toString() ?? '');
  final fundingRequiredController = TextEditingController(text: financialPlan['Funding_Required']?.toString() ?? '');
  final expectedRevenueController = TextEditingController(text: financialPlan['Expected_Revenue_First_Year']?.toString() ?? '');
  
  final teamSizeController = TextEditingController(text: operationalPlan['Team_Size']?.toString() ?? '');
  final resourcesController = TextEditingController(text: operationalPlan['Resources_Required'] ?? '');
  final timelineController = TextEditingController(text: operationalPlan['Timeline_To_Launch'] ?? '');
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        title: Text('Edit Business Information', style: TextStyle(color: Colors.white)),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              _buildEditTextField('Business Name', businessNameController),
              _buildEditTextField('Business Sector', businessSectorController),
              _buildEditTextField('Business City', businessCityController),
              _buildEditTextField('Business Location', businessLocationController),
              _buildEditTextField('Idea Description', ideaDescriptionController),
              _buildEditTextField('Target Market', targetMarketController),
              _buildEditTextField('USP', uspController),
              _buildEditTextField('Startup Cost', startupCostController, isNumeric: true),
              _buildEditTextField('Funding Required', fundingRequiredController, isNumeric: true),
              _buildEditTextField('Expected Revenue', expectedRevenueController, isNumeric: true),
              _buildEditTextField('Team Size', teamSizeController, isNumeric: true),
            ...
            onPressed: () {
              final updatedData = {
                'business': {
                  ...
                  'financialPlan': {
                    'Estimated_Startup_Cost': int.tryParse(startupCostController.text) ?? 0,
                    'Funding_Required': int.tryParse(fundingRequiredController.text) ?? 0,
                    'Expected_Revenue_First_Year': int.tryParse(expectedRevenueController.text) ?? 0,
                  },
                  'operationalPlan': {
...
void _showEditFinancialDialog() {
  ...
  final primaryIncomeController = TextEditingController(text: incomeDetails['Primary_Monthly_Income']?.toString() ?? '');
  final additionalIncomeController = TextEditingController(text: incomeDetails['Additional_Monthly_Income']?.toString() ?? '');
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Color(0xFF283593),
        title: Text('Edit Financial Information', style: TextStyle(color: Colors.white)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildEditTextField('Primary Monthly Income', primaryIncomeController, isNumeric: true),
            _buildEditTextField('Additional Monthly Income', additionalIncomeController, isNumeric: true),
          ],
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              final updatedData = {
                'financial': {
                  'incomeDetails': {
                    'Primary_Monthly_Income': int.tryParse(primaryIncomeController.text) ?? 0,
                    'Additional_Monthly_Income': int.tryParse(additionalIncomeController.text) ?? 0,
                  }
                }
              };
...
void _showEditAssetsDialog() {
  ...
  final goldAmountController = TextEditingController(text: assetDetails['Gold_Asset_amount']?.toString() ?? '');
  final goldValueController = TextEditingController(text: assetDetails['Gold_Asset_App_Value']?.toString() ?? '');
  final landAreaController = TextEditingController(text: assetDetails['Land_Asset_Area']?.toString() ?? '');
  final landValueController = TextEditingController(text: assetDetails['Land_Asset_App_Value']?.toString() ?? '');
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Color(0xFF283593),
        title: Text('Edit Assets Information', style: TextStyle(color: Colors.white)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildEditTextField('Gold Amount (grams)', goldAmountController, isNumeric: true),
            _buildEditTextField('Gold Value', goldValueController, isNumeric: true),
            _buildEditTextField('Land Area (sq ft)', landAreaController, isNumeric: true),
            _buildEditTextField('Land Value', landValueController, isNumeric: true),
          ],
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              final updatedData = {
                'financial': {
                  'assetDetails': {
                    'Gold_Asset_amount': int.tryParse(goldAmountController.text) ?? 0,
                    'Gold_Asset_App_Value': int.tryParse(goldValueController.text) ?? 0,
                    'Land_Asset_Area': int.tryParse(landAreaController.text) ?? 0,
                    'Land_Asset_App_Value': int.tryParse(landValueController.text) ?? 0,
                  }
                }
              };
...
void _showEditLoanDialog({int? loanIndex}) {
  ...
  final monthlyPaymentController = TextEditingController(text: loan['Monthly_Payment']?.toString() ?? '');
  final lenderNameController = TextEditingController(text: loan['Lender_Name'] ?? '');
  final loanTypeController = TextEditingController(text: loan['Loan_Type'] ?? '');
  final totalAmountController = TextEditingController(text: loan['Total_Loan_Amount']?.toString() ?? '');
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: Color(0xFF283593),
        title: Text(
          loanIndex != null ? 'Edit Loan Details' : 'Add New Loan',
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildEditTextField('Total Loan Amount', totalAmountController, isNumeric: true),
              _buildEditTextField('Monthly Payment', monthlyPaymentController, isNumeric: true),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              final updatedLoan = {
                'Monthly_Payment': int.tryParse(monthlyPaymentController.text) ?? 0,
                'Lender_Name': lenderNameController.text,
                'Loan_Type': loanTypeController.text,
                'Total_Loan_Amount': int.tryParse(totalAmountController.text) ?? 0,
              };
...
Widget _buildEditTextField(String label, TextEditingController controller, {bool isNumeric = false}) {
  return Padding(
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildSectionCard('Business Information', _buildBusinessDetails(), onEdit: _showEditBusinessDialog, onicon: Icon(Icons.edit, color: Color(0xFFFFC107), size: 20)),
                      SizedBox(height: 16),
                      
                      // Financial Information
                      _buildSectionCard('Financial Information', _buildFinancialDetails(), onEdit: _showEditFinancialDialog, onicon: Icon(Icons.edit, color: Color(0xFFFFC107), size: 20)),
 
                      SizedBox(height: 16),
                      
                      // Assets Information
                      _buildSectionCard('Assets', _buildAssetsDetails(), onEdit: _showEditAssetsDialog, onicon: Icon(Icons.edit, color: Color(0xFFFFC107), size: 20)),
                      SizedBox(height: 16),
 ...
 Widget _buildSectionCard(String title, Widget content, {VoidCallback? onEdit,
 Widget? onicon,}) {
  return Container(
    decoration: BoxDecoration(
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Column(
            children: [
              Text('No existing loans', style: TextStyle(color: Colors.white70)),
              SizedBox(height: 12),
              ElevatedButton.icon(
                onPressed: () => _showEditLoanDialog(),
                icon: Icon(Icons.add, color: Colors.black),
                label: Text('Add Loan', style: TextStyle(color: Colors.black)),
                style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFFFC107)),
              ),
            ],
          ),
    ...
    children: [
      ...
      Align(
        ...
        child: ElevatedButton.icon(
          ...
          label: Text('Add Loan', style: TextStyle(color: Colors.black, fontSize: 12)),
      ...
      Container(
        ...
        child: Column(
          children: financial['existingloanDetails'].asMap().entries.map<Widget>((entry) {
            int index = entry.key;
            ...
            return Column(
              children: [
                ...
                if (index < financial['existingloanDetails'].length - 1) ...[
                  SizedBox(height: 16),
                  Divider(color: Color(0xFFFFC107).withOpacity(0.3), thickness: 1),
                  SizedBox(height: 16),
                ],
              ],
            );
